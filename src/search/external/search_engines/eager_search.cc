...
namespace eager_search {
    ...
    void EagerSearch::initialize() {
        ...
        closed_list->find_insert(initial_state);
    ...
    SearchStatus EagerSearch::step() {
        ...
        */
        ...
        for (OperatorID op_id : applicable_ops) {
            ...
            bool found, reopened;
            std::tie(found, reopened) = closed_list->find_insert(succ_state);
            
            if (found && !reopened) continue; // in closed node
    ...
    pair<GlobalState, bool> EagerSearch::fetch_next_node() {
        while (true) {
            if (open_list->empty()) {
    ...
    bool EagerSearch::check_goal_and_set_plan(const GlobalState &state) {
        if (test_goal(state)) {
    ...
    void EagerSearch::update_f_value_statistics(const GlobalState &state) {
        if (f_evaluator) {
            /*
